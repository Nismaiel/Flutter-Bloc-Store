...
Widget favoritesButtonInfo(Product product) {
  return BlocBuilder<FavoritesCubit, FavoritesState>(builder: (context, state) {
    return GestureDetector(
      onTap: () {
//Todo: apply the conditions on the {onTap}
        state.favoritesList.forEach((element) {
          if (element.product.id == product.id) {
            context.read<FavoritesCubit>().removeFromFavorites(state,element.favKey,element.product);}else{
             context.read<FavoritesCubit>().addToFavorites(product, state, DateTime.now().microsecondsSinceEpoch.toString());}

        });
      },
      child: Card(
        color: state.favoritesList.contains(product)
...
//       if (state.favoritesList!.products != null &&
//           state.favoritesList.products.contains(product)) {
//         return IconButton(
//           onPressed: () {
              ...
              onPressed: !state.cartItems.products.contains(product) && !enabled
                  ...
                  : () {
                      state.cartItems.products.contains(product)
                          ? context
                              .read<CartCubit>()
                              .removeFromCart(state.cartItems.id)
                          : context
                              .read<CartCubit>()
                              .addToCart(size, color, product);
                    },
              child: Container(
